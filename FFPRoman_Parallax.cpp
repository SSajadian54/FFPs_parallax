...
struct source{
    ...
    double pos1a, pos2a, pos1b, pos2b, pos1c, pos2c, Astar, xi, ux, uy, ux0, uy0;
...
///===========================================================
    ...
    double timp1,lonn, tim;
    double chi1, chi2, chi3,chi1a, chi2a, chi3a, sil; 
    double dchi1, dchi2, dchi1a, dchi2a; 
...
///*********************************************************************************   
    ...
    ///######################################################################### 
    //dchi2= fabs( chi2- chi1)/ndw;
    //dchi2a=fabs(chi2a-chi1a)/ndw; 
    dchi1= fabs( chi3- chi1);
    //dchi1a=fabs(chi3a-chi1a);
    //cout<<"dchi_photo:    "<<dchi1<<"\t    dchi_astro :   "<<dchi1a<<endl;
   ...
   "%d  %d %.1lf  %d  %d  %.9lf  %.9lf   %.9lf  %.9lf  %.9lf   %.6lf %.8lf %.8lf  %.8lf  %.8lf  %.5lf %.5lf  %.5lf %.5lf\n",//57
   counter,s.lat, s.lon, //3
   l.struc, double(l.Ml*Mjupiter/Mearth), l.Dl, l.vl, //7
   ...
   flagf,flag_det,dchi1,ndw,li,s.mus1, s.mus2, s.xi,s.mul1,s.mul2,l.piE,s.ampM,s.errM,s.ampA,s.errA,chi1,chi2,chi3,l.phiP*RA);//57
   fclose(fil1);
...
///#############################################################################  
   ...
   for(int k=(j+1);k<Nx; ++k) co.FishM[j][k]= co.FishM[k][j]; }
    ...
    fil2=fopen("./files/MONT1/FFP_Roman2c.dat","a+");
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH   
   ...
   lightcurve(s,l,as,timn[i]);
   dera1[h]= float(s.pos1c-soux[i])/(Delta2[j]*sig[h]);  
   derb1[h]= float(s.pos2c-souy[i])/(Delta2[j]*sig[h]);
   if(j==0) l.tetE -= double(Delta2[0]*sig[h]);
   if(j==1) s.mus1 -= double(Delta2[1]*sig[h]); 
   if(j==2) s.mus2 -= double(Delta2[2]*sig[h]);
   if(j==3)  l.piE -= double(Delta2[3]*sig[h]);}
   ...
   lightcurve(s,l,as,timn[i]);
   dera2[h]= float(s.pos1c-soux[i])/(Delta2[k]*sig[h]);  
   derb2[h]= float(s.pos2c-souy[i])/(Delta2[k]*sig[h]);  
   if(k==0) l.tetE -= double(Delta2[0]*sig[h]);
   if(k==1) s.mus1 -= double(Delta2[1]*sig[h]); 
   if(k==2) s.mus2 -= double(Delta2[2]*sig[h]);
   if(k==3)  l.piE -= double(Delta2[3]*sig[h]); }
   ...
   fil3=fopen("./files/MONT1/FFP_Roman3c.dat","a+");
...
{ 
   ...
   //vex= as.vearth*cos(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP)*cos(as.tetp);//-U
   //vey= as.vearth*sin(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP)//-V
   //vez= as.vearth*cos(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP)*sin(as.tetp);//W
   
   dvex= +as.vearth*sin(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP)*cos(as.tetp)/as.omegae;///by taking integrations
   dvey= -as.vearth*cos(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP) / as.omegae;
   dvez= +as.vearth*sin(as.omegae*(tt-l.t0)+M_PI/2.0 +l.phiP)*sin(as.tetp)/as.omegae;
  
   as.Ve_n1= -dvex*sin(s.deltao) + dvey*cos(s.deltao);
   Ve_x=     +dvex*cos(s.deltao) + dvey*sin(s.deltao);
   as.Ve_n2= -sin(s.FI)*Ve_x + cos(s.FI)*dvez;
   ...
   s.ux0=-l.u0*sin(s.xi) + (timh-l.t0)*cos(s.xi)/l.tE ;
   s.uy0= l.u0*cos(s.xi) + (timh-l.t0)*sin(s.xi)/l.tE ;   
      
   s.ux= s.ux0 + l.piE * as.ue_n1;
   s.uy= s.uy0 + l.piE * as.ue_n2;
   
   s.ut0=sqrt( s.ux0*s.ux0 + s.uy0*s.uy0 );
   s.ut= sqrt( s.ux*s.ux   +  s.uy*s.uy);  
   
   if(s.ut==0.0)   s.ut=1.0e-50; 
   if(s.ut0==0.0)  s.ut0=1.0e-50; 
   
   s.def1a=  s.ux0*l.tetE/(s.ut0*s.ut0 + 2.0);//x-deflection, without parallax
   s.def2a=  s.uy0*l.tetE/(s.ut0*s.ut0 + 2.0);//y-deflection, without parallax 
   
   //s.def1b=  s.ux0*l.tetE/(s.ut*s.ut + 2.0); //x-deflection, first sentence
   //s.def2b=  s.uy0*l.tetE/(s.ut*s.ut + 2.0); //y-deflection, first sentence 
   
   s.def1c=  s.ux*l.tetE/(s.ut*s.ut + 2.0); //x-deflection
   s.def2c=  s.uy*l.tetE/(s.ut*s.ut + 2.0); //y-deflection 
  
  
   s.pos1a= -l.u0*l.tetE*sin(s.xi) + s.mus1*(timh-l.t0) - as.ue_n1*pis ;//x-source trajectory, no lensing 
   s.pos2a= +l.u0*l.tetE*cos(s.xi) + s.mus2*(timh-l.t0) - as.ue_n2*pis ;//y-source trajectory, no lensing  
  
   s.pos1b= -l.u0*l.tetE*sin(s.xi) + s.mus1*(timh-l.t0) + s.def1a;//x-source trajectory, without parallax  
   s.pos2b= +l.u0*l.tetE*cos(s.xi) + s.mus2*(timh-l.t0) + s.def2a;//y-source trajectory, without parallax  
  
   s.pos1c= -l.u0*l.tetE*sin(s.xi) + s.mus1*(timh-l.t0) - as.ue_n1*pis  +s.def1c;//x-source trajectory && parallax
   s.pos2c= +l.u0*l.tetE*cos(s.xi) + s.mus2*(timh-l.t0) - as.ue_n2*pis  +s.def2c;//y-source trajectory && parallax 
}
///==============================================================//6
///                                                              //
///                  optical_depth                               //
...
void func_lens(lens & l, source & s)
{
    ...
    l.rhomaxl=0.0;
    int il, yye;
    double mmin=0.01*Mearth/Mjupiter;//in jupiter mass
    double mmax=15.0;///in jupiter mass 
   ...
   double randflag=RandR(0.0,s.Rostar0[l.numl]);
    ...
    f=double(1.0/l.Ml); 
    test = RandR(1.0/mmax, 1.0/mmin);//log uniform
    }while(test>f);


    l.xls=l.Dl/s.Ds;
    l.RE=sqrt(4.0*G*l.Ml*Mjupiter*s.Ds*KP)/velocity;
    ...
    l.phiP=RandR(1.0 , 359.0)*pi/180.0; 
...
double RandN(double sigma, double nnd){
   double rr,f,frand;
   do{
   rr=RandR(-1.0*sigma*nnd, sigma*nnd);
